#include "main.h"

/**
 * use_binary - this converts an int to binary
 * @arg: holds the value being displayed
 * Return: the length of displayed value
 */

int use_binary(va_list arg)
{
// variable declaration

long int divisor = 2, len = 0, i, j, k, newval;
int *ptr;
unsigned int val = va_arg(arg, int);

if (val == 0)
_putchar ('0');
len++;
return (len);
}
newval = val;
i = 0;
while (newval > 0)
{
newval /= divisor;
i++;
}
// using malloc to create space for the length of numbers
ptr = malloc(sizeof(int) * i);
if (ptr == NULL)
return (-1);
for (j = 0; j < i; j++)
{
ptr[j] = val % divisor;
val /= divisor;
}
// print the numbers in reverse by decrementing the value of k
for (k = j - 1; k >= 0; k--)
_putchar(ptr[k] + '0');
free(ptr);
return (i);
}
